...
uncompress' w (x:xs) dict output
  | x < length dict = uncompress' (valueAtIndex x dict) xs (dict++[w++[head (valueAtIndex x dict)]]) (output++(valueAtIndex x dict))
  | otherwise = uncompress' (w++[head w]) xs (dict++[w++[head (w++[head w])]]) (output++(w++[head w]))